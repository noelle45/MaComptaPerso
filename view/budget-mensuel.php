...
//-------------- PREVISIONNEL ---------------------------------------------------------------------  
                    ...
                    <p class="black"> <a class="black a-hover-blue" href="http://myspacefamily.fr/MaComptaPerso/view/budget-mensuel.php?action=creabudget">
                    <strong><p class="white2 size22">
                                    ...
                                    while($data_charges=$query_charges->fetch())
                                    {
                                        if($data_charges['id_budget']==$id_budget)
                                        {
                                            echo'<tr>';
                                               ...
                                               // echo'</td>';
                                                ...
                                                echo'<td>';
                                                    ...
                                                    {
                                                        echo'<p class="black size12">' . $data_charges['montant'] . '</p>';
                                            ...
                                            echo'<tr>
                                                ...
                                                <td colspan="2" class="size15">'. number_format($totalDebit, 2, ',', ' ') .
                                               '</td>
                                               <td colspan="2" class="size15">'. number_format($totalCredit, 2, ',', ' ') .
                                               '</td>
                                            </tr>
...
//----CHARGES FIXES DEBIT -------------------------------------------------------------------------------
                        ...
                        $query1=$db->prepare('SELECT * FROM charges_fixes
                        WHERE id_createur=:id'); 
                        $query1->bindValue(':id', $_SESSION['id'], PDO::PARAM_INT);
                        $query1->execute();
                        $dat_crea = $query1->fetch();
                        $createur = $dat_crea['id'];

                        $query2=$db->prepare('SELECT sum(`montant`) FROM charges_fixes 
                        WHERE debit_credit="D" 
                        AND id_createur=:id
                        AND archive=0
                        ');
                        $query2->bindValue(':id', $_SESSION['id'], PDO::PARAM_INT);
                        $query2->execute();
                        $resultDCF=$query2->fetch();
                        echo'
...
//---- CHARGES FIXES CREDIT -------------------------------------------
                        $query3=$db->prepare('SELECT sum(`montant`) FROM charges_fixes
                        WHERE debit_credit="C" 
                        AND id_createur=:id
                        AND archive=0
                        ');
                        $query3->bindValue(':id', $_SESSION['id'], PDO::PARAM_INT);
                        $query3->execute();
                        $resultCCF=$query3->fetch();
                
//---- SOLDES CREDITS -------------------------------------------               
                        $creditTC = $resultCCF[0];
                        echo'
                        <tr class="text-left">
                            <td>
                        ...
                        $soldeB = ($creditTC - $resultDCF[0]);
                        echo'
                        
                        <tr class="text-left bg-blue bold" style="border:1px solid white">
                                <td colspan="2" class="text-align-center">
                                    <p class="black p-3">Solde sur votre prévisonnel (Budget) 
                                    <br/><br/> '. number_format($soldeB, 2, ',', ' ') .' €</p>
                                </td>
                            </tr>
                    </table>';
...
//---- DEBITS --------------------------------------- 
                ...
                $mois_en_cours = $table2[$_GET['month']];
                        ...
                        $query5=$db->prepare('SELECT sum(`montant`), date_ecriture
                        FROM ecritures 
                        WHERE debit_credit="D"
                        ...
                        AND pointer=1
                        AND id_createur=:id');
                        $query5->bindValue(':anne_en_cours', $_GET['year'], PDO::PARAM_STR);
                        $query5->bindValue(':mois_en_cours',$mois_en_cours, PDO::PARAM_STR);
                        $query5->bindValue(':id',$_SESSION['id'], PDO::PARAM_INT);
                        $query5->execute();
                        $debit = $query5->fetch();
...
//---- CREDITS -------------------------------------------
                        ...
                        $query6=$db->prepare('SELECT sum(`montant`), date_ecriture
                        FROM ecritures
                        WHERE debit_credit="C" 
                        AND YEAR(date_ecriture)=:anne_en_cours
                        AND MONTH(date_ecriture)=:mois_en_cours
                        AND pointer=1
                        AND id_createur=:id');
                        $query6->bindValue(':anne_en_cours', $_GET['year'], PDO::PARAM_STR);
                        $query6->bindValue(':mois_en_cours',$mois_en_cours, PDO::PARAM_STR);
                        $query6->bindValue(':id',$_SESSION['id'], PDO::PARAM_INT);
                        $query6->execute();
                        $credit = $query6->fetch();
...
//---- RESULTAT ------------------------------------------
                        ...
                        $query7=$db->prepare('SELECT montant, date_ecriture
                        FROM ecritures
                        WHERE debit_credit="C" 
                        AND pointer =0
                        AND id_createur=:id');
                        ...
                        $query8=$db->prepare('SELECT montant, date_ecriture
                        FROM ecritures
                        WHERE debit_credit="D" 
                        AND pointer =0
                        AND id_createur=:id');
                        
                        $query8->bindValue(':id',$_SESSION['id'], PDO::PARAM_INT);
                        $query8->execute();
                        ...
                        echo'<a href="http://myspacefamily.fr/MaComptaPerso/view/mes-comptes.php"><p class="bold white2 bg-green p-2 text-align-center">Voir le détail de mes comptes</p></a>';
                        echo'
                            <table class="w-100">
                            <tr class="text-left">
                                <td>
...
//---- DEBITS --------------------------------------- 
                        ...
                        AND pointer=1
                        AND id_createur=:id');
                        $querydebit->bindValue(':id',$_SESSION['id'], PDO::PARAM_INT);
                        $querydebit->execute();
...
//---- CREDITS -------------------------------------------
                        ...
                        $querycredit->bindValue(':id',$_SESSION['id'], PDO::PARAM_INT); 
                        $querycredit->execute();
...
//-------------------------------------- END VIEW ------------------------------
                ...
                <div class="row">
                	<div class="col">
                    	<div class="card card50 ombre mt-5 p-5">
                      		<form action="http://myspacefamily.fr/MaComptaPerso/view/budget-mensuel.php?action=creabudgetok" method="post">
                      			<table style="margin-top:-20px">
                          			<tr>
                              			<td>Donnez un nom à votre nouveau budget<br/>
                              			<i class="italic size15"> exemple : Charges mensuelles fixes </i><br/><br/>
                                  			<input type="text" name="nom_budget" /> <input type="submit" value="Créer!"/>
                              			</td>
                          			</tr>
                      			</table>
                      		</form>
            ...
            case "creabudgetok":
                ...
                $query=$db->prepare('INSERT INTO `budgets`(`id_createur`, `nom_budget`)
                VALUES (:id_createur, :nom_budget)');

                $query->bindValue(':id_createur', $id_createur, PDO::PARAM_INT);
                $query->bindValue(':nom_budget', $nom_budget, PDO::PARAM_STR);
                $query->execute();
            
            echo'<meta http-equiv="refresh" content="1;http://myspacefamily.fr/MaComptaPerso/view/budget-mensuel.php?month='.$monyth.'&year='.$year.'">';
...
//--------------- CREATE PREVISIONNEL -----------------------------------------
            ...
            $year=date('Y');
            echo'                
            <a class="white2" href="http://myspacefamily.fr/MaComptaPerso/view/budget-mensuel.php?month='.$monyth.'&year='.$year.'">
...
//---------------------------- CREATE PREVISIONNEL OK ---------------------------------------
            ...
            <div class="card mt-5 card50 ombre bg-white-diffu">
            <a class="white2 bg-green-diffu border-radius-zig a-hover-rose" href="budget-mensuel.php?action=creaprev&budget='.$id_budget.'">
            <p class="p-3 white2">Saisir une autre charges</p></a><br/>
            <a class="white2 bg-green-diffu border-radius-zig a-hover-rose mt-3" href="http://myspacefamily.fr/MaComptaPerso/view/budget-mensuel.php?month='.date('m').'&year='.date('Y').'">
            <p class="p-3 white2">Retour à votre budget</p></a>
            </div>
            </div>';
...
//----------------------------------------------- ARCHIVER ------------------------------------------------------
 ...
 echo'<meta http-equiv="refresh" content="1;http://myspacefamily.fr/MaComptaPerso/view/budget-mensuel.php?month='.date('m').'&year='.date('Y').'">';
 break;
//---------------------------------------------------------------------------------------------
